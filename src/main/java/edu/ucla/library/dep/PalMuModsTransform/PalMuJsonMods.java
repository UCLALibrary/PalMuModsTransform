...
public class PalMuJsonMods {
	...
	public static void main(String[] args) throws IOException {
		...
		//byte[] jsonData = Files.readAllBytes(Paths.get("jsonPalmu.json"));
		//byte[] jsonDataAr = Files.readAllBytes(Paths.get("jsonPalmuAr.json"));
        byte[] jsonData =  Files.readAllBytes(Paths.get("export-en.json"));
		...
		//PalMuJson palmuJson = new ObjectMapper().readerFor(PalMuJson.class).readValue(jsonData);
		//PalMuJson palmuJsonAr = new ObjectMapper().readerFor(PalMuJson.class).readValue(jsonDataAr);
	    //PalMuRecord[] records = palmuJson.getResults();
	    //PalMuRecord[] recordsAr = palmuJsonAr.getResults();
	    for (int i = 0; i < records.length; i++) {
			...
			if(null != record.getPreview_url() && record.getPreview_url().length > 0){
				...
				Element childIdentifier = new Element("identifier", namespace);
				childIdentifier.setAttribute("type", "uri");
			    childIdentifier.addContent(record.getPreview_url()[0]);
			...
			if(null != record.getMedium_url() && record.getMedium_url().length > 0) {
				...
				Element childIdentifier = new Element("identifier", namespace);
				childIdentifier.setAttribute("type", "uri");
			    childIdentifier.addContent(record.getMedium_url()[0]);
			...
			xmlOutput.output(jdomDoc,
					new FileWriter("\\\\svm_dlib\\DLIngest\\palmu\\Dec10-2019\\"
